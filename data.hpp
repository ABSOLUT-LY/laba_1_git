...
public: // доступно вне класса
    ...
    Data() : n(0), arr(nullptr), negative_count(0) {} // через ":" мы инициализируем переменные сразу какие они нам нужны, тело {} пустое, так как мы уже присвоили им санчала нужные данные и действий не требуется
    // деструктор
    ~Data()
    {
